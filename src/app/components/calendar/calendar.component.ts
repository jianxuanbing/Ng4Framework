...
export class CalendarComponent implements OnInit{
    ...
    getMonthDay(year:number,month:number):any{
        //获取当月有多少天，当月第一天是星期几
    ...
    getSomeDay(d:any,add:number):object{
        //获取前一天或后一天
    ...
    drawCalendar(selectYear='',selectMonth=''){
        ...
        if(selectYear===''){
            let calendar='<div id="nCalendar" class="calendar" style="width:'+this.width+'px;"><div class="calendar-tool">';
            ...
            calendar+='<i class="ion ion-chevron-right"></i></div></div>';
        ...
        const cc=document.querySelector('.calenday-body');
        cc.innerHTML='';
    ...
    addClickEvent(){
        ...
        // 为天数添加点击事件
        this.items=document.querySelector('.calendar-body').querySelectorAll('.item:not(.pass):not(.future)');
